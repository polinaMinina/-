...
@time: 2021/06/19
@file: smallNORB.py
@describe:This file aims to load smallNORB data set.
"""
import torch
...
class smallNORB(data.Dataset):
    """
    MNIST <https://cs.nyu.edu/~ylclab/data/norb-v1.0-small//> Dataset.
    Args:
        root (string): Root directory of dataset where processed folder and raw folder exist.
        train (bool, optional): If True, creates dataset from the training files, otherwise from the test files.
        download (bool, optional): If true, downloads the dataset from the internet and puts it in root directory.
            If the dataset is already processed, it is not processed and downloaded again.
        transform (callable, optional): A function/transform that takes in an PIL image and returns a transformed
            version. E.g, ``transforms.RandomCrop``
        target_transform (callable, optional): A function/transform that takes in the target and transforms it.
        info_transform (callable, optional): A function/transform that takes in the info and transforms it.
...
class smallNORBViewPoint(data.Dataset):
    ...
    def __init__(self, root, exp='azimuth', train=True, familiar=True, transform=None, target_transform=None,
                 info_transform=None, download=False,
                 mode="all"):
